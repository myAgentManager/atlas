...
// Service lock: when Operations flips it on, everyone but staff gets the lock
// message. Sign-in stays open (staff need it), public info stays readable, and
// the VoIP webhook handles the lock itself so phone systems get a spoken line.
app.use('/api', (req, res, next) => {
...
app.patch('/api/me', auth.requireAuth, (req, res) => {
  ...
  auth.updateUser(req.user.id, (u) => {
    ...
    // setup only counts as finished once 2SV is enrolled — no skipping the
    // security step by poking the API or bailing out of the wizard early
    if (welcomed === true && auth.secondMethod(u)) u.welcomed = true;
...
// Atlas Support: generate a one-hour code that lets staff open an agent-only
// support view of your account (agents, capabilities, knowledge counts) —
// never your conversations, customers, or files.
app.post('/api/me/support-code', auth.requireAuth, (req, res) => {
  auth.audit('auth', `support code generated by ${req.user.email}`);
  res.json({ code: auth.createSupportCode(req.user.id), expiresMin: 60 });
});
...
app.post('/api/agents/:id/message', auth.requireAuth, (req, res) => {
  ...
  const reply = agents.handle(req.user.id, agent, text, { greeted, channel, history });
...
async function runEmailForUser(user) {
  ...
  for (const m of msgs) {
    ...
    // A returning sender continues their thread: the agent remembers what it
    // already told them (no re-greeting, no reciting the hours again).
    let convo = biz.listInbox(user.id).find((c) => c.channel === 'email' && c.customerEmail === senderEmail);
    const greeted = Boolean(convo && convo.messages.some((x) => x.from === 'agent'));
    const history = (convo?.messages || []).filter((x) => x.from === 'agent').slice(-6).map((x) => x.text);
    if (!convo) convo = biz.openConversation(user.id, { channel: 'email', agentId: agent.id, customer: customer.name, customerEmail: senderEmail, subject: m.subject });
    biz.addMessage(user.id, convo.id, 'customer', m.text || '(no text)');
    const reply = agents.handle(user.id, agent, `${m.subject} ${m.text}`, { greeted, channel: 'email', history });
...
app.post('/api/voip/ivr', express.urlencoded({ extended: false }), (req, res) => {
  ...
  if (getPlatform().locked?.enabled) {
    const out = { say: 'This service is temporarily unavailable. Please call back later.', hangup: true };
    return (b.format === 'xml' || /xml/i.test(req.headers.accept || '')) ? res.type('text/xml').send(voip.toXml(out)) : res.json(out);
  }
...
// Email agent loop: every minute (MYAGENT_EMAIL_MINUTES to change), any account
// with an inbox connector and an active email agent gets its new mail read +
// answered automatically. First pass runs shortly after boot.
const EMAIL_MIN = Number(process.env.MYAGENT_EMAIL_MINUTES ?? 1);
let pollingEmail = false;
async function emailTick() {
  if (pollingEmail || EMAIL_MIN <= 0 || getPlatform().locked?.enabled) return;
  pollingEmail = true;
  try {
...
if (EMAIL_MIN > 0) { setTimeout(emailTick, 20_000); setInterval(emailTick, EMAIL_MIN * 60_000); }

// Morning digest — each owner gets yesterday's numbers + top knowledge gap.
setInterval(() => { if (!getPlatform().locked?.enabled) digestTick().catch(() => {}); }, 60 * 60_000);
