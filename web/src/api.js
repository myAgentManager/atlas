...
export const api = {
  ...
  joinTeam: (code) => post('/api/team/join', { code }),
  removeTeamMember: (id) => send(`/api/team/member/${id}`, 'DELETE'),
  transferOwner: (toId) => post('/api/team/transfer', { toId }),
  voipSetup: () => get('/api/voip/setup'),
  testPbx: () => post('/api/connectors/pbx/test'),
  testPbxCall: (toExt) => post('/api/connectors/pbx/testcall', { toExt }),
  parseSipLine: (line) => post('/api/connectors/pbx/parse', { line }),
